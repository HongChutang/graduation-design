...
class Model(dict):
    ...
    def get(cls, pk):
        ...
        Get by primary key.
        '''
        d = db.select_one('select * from %s where %s="%s"' % (cls.__table__, cls.__primary_key__.name, pk))
    ...
    def update(self):
        ...
        sql = 'update `%s` set %s where %s="%s"' % (self.__table__, ','.join(L), pk, pk_val)
        db.update(sql)
        return self
    ...
    def delete(self):
        ...
        sql = 'delete from `%s` where `%s`="%s"' % (self.__table__, pk, args[0])
        db.update(sql)
        return self
...
if __name__=='__main__':
    ...
    db.create_engine('root', '694776425', 'pytest')
