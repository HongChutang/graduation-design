...
def blog(blog_id):
    ...
    comments = Comment.find_by('where blog_id="%s" order by created_at desc limit 1000' % blog_id)
...
def authenticate():
    ...
    password = i.password
    remember = i.remember
    user = User.find_first('where email="%s"' % email)
    if user is None:
...
def register_user():
    ...
    user = User(name=name, admin=0, email=email, password=password,
...
def _get_blogs_by_page():
    ...
    blogs = Blog.find_by('order by created_at desc limit %s, %s' % (page.offset, page.limit))
    return blogs, page
...
def api_get_comments():
    ...
    page = Page(total, _get_page_index())
    comments = Comment.find_by('order by created_at desc limit %s, %s' % (page.offset, page.limit))
...
def api_get_users():
    ...
    page = Page(total, _get_page_index())
    users = User.find_by('order by created_at desc limit %s,%s' % (page.offset, page.limit))
    for u in users:
        u.password = '******'
...
# sql注入检测拦截器
import sql_injection_ds.core as sqlInjectionDSCore
@interceptor('/')
def sql_injection_ds_interceptor(next):
    ...
    # 如果有参数，则进一步检测
    logging.info('try to detect sql injection...')

    # 如果检测结果为攻击，则返回404
    if sqlInjectionDSCore.predict(ctx.request):
        raise notfound()
